use crate::{
    ast::Token,
    std::{
        core::{io::StdCoreIo, load_core},
        literal::{load_literal, number::StdLiteralNumber, string::StdLiteralString},
    },
    utils::errors::raw,
};
    ...
    fn load_core_io(&self, names: Vec<(Token, Option<Token>)>, all: bool) {
        ...
        for (name, name2) in names {
            match name.lexeme.as_str() {
                ...
                _ => raw(format!("std module 'core::io::{}' doesn't exist", name.lexeme).as_str()),
            }
        }
    }
